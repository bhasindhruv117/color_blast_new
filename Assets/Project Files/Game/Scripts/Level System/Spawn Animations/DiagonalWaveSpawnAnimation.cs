...
{
    [CreateAssetMenu(fileName = "Diagonal Wave Spawn Animation", menuName = "Data/Level/Diagonal Wave Spawn Animation")]
    public class DiagonalWaveSpawnAnimation : LevelAnimation
    {
        ...
        public class FadeTweenCase : TweenCase
        {
            ...
            public FadeTweenCase(LevelRepresentation levelRepresentation, float duration, float elementDelay, Color startColor, Color targetColor)
            {
                ...
                for (int i = 0; i < elements.Count; i++)
                {
                    ...
                    // Calculate the delay based on the distance from the left bottom corner
                    float distance = Mathf.Sqrt(elementPosition.x * elementPosition.x + elementPosition.y * elementPosition.y);
                    startTime[i] = distance * elementDelay;
